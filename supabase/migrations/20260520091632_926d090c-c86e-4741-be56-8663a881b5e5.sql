...
BEGIN
  ...
  NEW.outstanding := GREATEST(
    CASE
      ...
      ELSE ROUND((_total_sales - _total_payment_received - _commission_amount - _commission_with_invoice - _commission_without_invoice - COALESCE(NEW.adjustment, 0))::numeric, 2)
    END,
    0
  );
...
-- Backfill: trigger recalculation on all existing events
