...
BEGIN
  ...
  _ebitda := ROUND((_total_sales - _total_cost)::numeric, 2);
  ...
  NEW.ebitda_percent := CASE
    WHEN _total_sales > 0 THEN ROUND(((_ebitda / _total_sales) * 100)::numeric, 2)
    ELSE 0
  END;
...
-- Backfill all existing events with the new EBITDA formula
UPDATE public.events
SET ebitda = ROUND((total_sales - total_cost)::numeric, 2),
    ebitda_percent = CASE WHEN total_sales > 0
      THEN ROUND((((total_sales - total_cost) / total_sales) * 100)::numeric, 2)
      ELSE 0 END;
